...
> ### Ernest's Notes
> * **Updates**:
>   * Added personal notes to various Jupyter notebooks.
>   * Updated multiple scripts and notebooks to support Qiskit 2.5.0 (migrated from Qiskit 1.0.0).
>   * Fixed MathJax rendering: Updated LaTeX macro definitions natively in all 82 notebooks so that matrices (`\myvector` using `pmatrix`) and states (`\pstate` using `\ket`) render automatically on load, removing the need for deprecated/unreliable `MathJax.Hub.Config` Python/JavaScript helper cells.
